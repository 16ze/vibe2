      ...
      <div className="flex-1 min-w-0 text-left">
        ...
        {isTyping ? (
          <div className="flex items-center gap-1.5 mt-0.5">
            <span className="text-xs text-gray-500 italic">En train d'écrire</span>
            <TypingIndicator />
          </div>
        ) : (
          <div className="flex items-center gap-2 mt-0.5">
            ...
            {hasUnread && (
              <div className={`flex-shrink-0 w-2 h-2 ${colors.bg} rounded-full`} />
            )}

            {/* Icône de statut */}
            <div
              className={`flex-shrink-0 w-3 h-3 ${
                hasUnread ? colors.bg : `border-2 ${colors.border} bg-transparent`
              } rounded-sm`}
            />
